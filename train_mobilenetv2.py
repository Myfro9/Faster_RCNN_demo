...
def main():
    ...
    VOC_root = "./"  # VOCdevkit
    # check voc root
    if os.path.exists(os.path.join(VOC_root, "PascalVOC2012")) is False:
        raise FileNotFoundError("PascalVOC2012 dose not in path:'{}'.".format(VOC_root))

    # load train data set
    # PascalVOC2012 -> VOC2012 -> ImageSets -> Main -> train.txt
    train_data_set = VOC2012DataSet(VOC_root, data_transform["train"], "train.txt")
    # 注意这里的collate_fn是自定义的，因为读取的数据包括image和targets，不能直接使用默认的方法合成batch
    batch_size = 8
    nw = min([os.cpu_count(), batch_size if batch_size > 1 else 0, 8])  # number of workers
    print('The number of CUP is : ', os.cpu_count())
